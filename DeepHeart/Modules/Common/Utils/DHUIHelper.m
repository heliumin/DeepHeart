...
@implementation DHUIHelper (QMUIMoreOperationAppearance)

+ (void)customMoreOperationAppearance {
    // 如果需要统一修改全局的 QMUIMoreOperationController 样式，在这里修改 appearance 的值即可
    [QMUIMoreOperationController appearance].cancelButtonTitleColor = UIColor.dh_tintColor;
}

@end


@implementation DHUIHelper (QMUIAlertControllerAppearance)
...
@implementation DHUIHelper (QMUIDialogViewControllerAppearance)
...
+ (void)customDialogViewControllerAppearance {
    ...
    appearance.backgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<DHThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:DHThemeIdentifierDark]) {
            return UIColorMake(34, 34, 34);
        }
        return UIColorWhite;
    }];
    appearance.headerViewBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<DHThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:DHThemeIdentifierDark]) {
            return UIColorMake(34, 34, 34);
        }
    ...
    appearance.headerSeparatorColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<DHThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:DHThemeIdentifierDark]) {
    ...
    buttonTitleAttributes[NSForegroundColorAttributeName] = UIColor.dh_tintColor;
    appearance.buttonTitleAttributes = [buttonTitleAttributes copy];
    
    appearance.buttonHighlightedBackgroundColor = [UIColor.dh_tintColor colorWithAlphaComponent:.25];
}

@end


@implementation DHUIHelper (QMUIEmotionView)
...
+ (void)customEmotionViewAppearance {
    ...
    [QMUIEmotionView appearance].sendButtonBackgroundColor = UIColor.dh_tintColor;
}

@end

@implementation DHUIHelper (QMUIImagePicker)
...
+ (void)customImagePickerAppearance {
    ...
    [QMUIImagePickerCollectionViewCell appearance].checkboxImage = [checkboxImage qmui_imageWithTintColor:UIColor.dh_tintColor];
    [QMUIImagePickerCollectionViewCell appearance].checkboxCheckedImage = [checkboxCheckedImage qmui_imageWithTintColor:UIColor.dh_tintColor];
    [QMUIImagePickerPreviewViewController appearance].toolBarTintColor = UIColor.dh_tintColor;
}

@end

@implementation DHUIHelper (UITabBarItem)
...
@implementation DHUIHelper (Button)
...
+ (QMUIButton *)generateDarkFilledButton {
    ...
    button.backgroundColor = UIColor.dh_tintColor;
    button.highlightedBackgroundColor = [UIColor.dh_tintColor qmui_transitionToColor:UIColorBlack progress:.15];// 高亮时的背景色
...
+ (QMUIButton *)generateLightBorderedButton {
    QMUIButton *button = [[QMUIButton alloc] qmui_initWithSize:CGSizeMake(200, 40)];
    button.titleLabel.font = UIFontBoldMake(14);
    button.tintColorAdjustsTitleAndImage = UIColor.dh_tintColor;
    button.backgroundColor = [UIColor.dh_tintColor qmui_transitionToColor:UIColorWhite progress:.9];
    button.highlightedBackgroundColor = [UIColor.dh_tintColor qmui_transitionToColor:UIColorWhite progress:.75];// 高亮时的背景色
    button.layer.borderColor = [button.backgroundColor qmui_transitionToColor:UIColor.dh_tintColor progress:.5].CGColor;
    button.layer.borderWidth = 1;
    button.layer.cornerRadius = 4;
    button.highlightedBorderColor = [button.backgroundColor qmui_transitionToColor:UIColor.dh_tintColor progress:.9];// 高亮时的边框颜色
    return button;
}
...
@implementation DHUIHelper (Emotion)
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (QMUIEmotion *e in emotions) {
            e.image = [UIImageMake(e.identifier) qmui_imageWithBlendColor:UIColor.dh_tintColor];
        }
    });
}
...
@implementation DHUIHelper (SavePhoto)
...
@implementation DHUIHelper (Calculate)
...
@implementation DHUIHelper (Theme)
