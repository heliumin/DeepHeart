...
extern NSString *const DHSelectedThemeIdentifier;
extern NSString *const DHThemeIdentifierDefault;
extern NSString *const DHThemeIdentifierGrapefruit;
extern NSString *const DHThemeIdentifierGrass;
extern NSString *const DHThemeIdentifierPinkRose;
extern NSString *const DHThemeIdentifierDark;

#define CodeFontMake(_pointSize) [UIFont fontWithName:@"Menlo" size:_pointSize]
#define CodeAttributes(_fontSize) @{NSFontAttributeName: CodeFontMake(_fontSize), NSForegroundColorAttributeName: DHThemeManager.currentTheme.themeCodeColor}

#define IsUITest NSProcessInfo.processInfo.environment[@"isUITest"].boolValue

/// QMUIButton 系列 Demo 里的一行高度
extern const CGFloat DHButtonSpacingHeight;

@interface DHCommonUI : NSObject

+ (void)renderGlobalAppearances;

@end

@interface DHCommonUI (ThemeColor)

+ (UIColor *)randomThemeColor;

@end

@interface DHCommonUI (Layer)
